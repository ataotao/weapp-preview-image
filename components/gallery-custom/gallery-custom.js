...
const app = getApp();
import { throttle, AutoSize, getSystemInfo } from "../preview-image/utils";
...
Component({
  ...
  properties: {
    imgUrls: {
      ...
      observer(newVal = [], oldVal, changedPath) {
        // console.log(newVal);
        const { contentWidth, contentHeight } =  this.data;
        this.setData({ 
          currentImgs: newVal.map(v => {
            return {
              ...v,
              imageX: (contentWidth) / 2,
              imageY: (contentHeight + getSystemInfo().customBar) / 2,
              orgImageX: (contentWidth ) / 2,
              orgImageY: (contentHeight + getSystemInfo().customBar) / 2
            }
          })
        });
      }
  ...
  data: {
    currentImgs: [],
    customBar: getSystemInfo().customBar,
    currentCount: 1, // 当前索引
    isLoading: false,
    _lastTapTime: 0,
    /**手势处理 */
  ...
  methods: {
    ...
    getImgInfo(currentCount) {
      return new Promise((resolve, reject) => {
        const { currentImgs, contentWidth, contentHeight } = this.data;
          ...
          wx.getImageInfo({
            ...
            success: (imgRes) => {
              ...
              _currentImgs[index] = ({
                ...
                imageY: (contentHeight - img.height + getSystemInfo().customBar) / 2,
                orgImageX: (contentWidth - img.width) / 2,
                orgImageY: (contentHeight - img.height + getSystemInfo().customBar) / 2
              });
              this.setData({ 
